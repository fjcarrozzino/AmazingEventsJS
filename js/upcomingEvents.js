...
function upcomingEvents(data) {
  ...
  if (data.length == 0) {
    templateHtml += `<h1 class="h1-nothing-found2"> Nothing was found. Please try with something else.</h1>`;
  }
  document.getElementById("div-upcoming-events").innerHTML = templateHtml;
}

const createCheckbox = () => {
  const checkboxes = document.getElementById("checkboxes");
  let fechaActual = dataArray.currentDate
  const dateFiltered = dataArray.events.filter((e) => fechaActual < e.date);
  const category = dateFiltered.map((eventos) => eventos.category);
